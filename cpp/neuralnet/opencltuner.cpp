...
//  return Global::strprintf("tune%d_gpu%s_x%d_y%d_c%d_mv%d.txt", TUNER_VERSION, gpuNameForFile.c_str(), nnXLen, nnYLen, model->trunk.trunkNumChannels,model->version);
  return Global::strprintf("tune%d_gpu%s_x%d_y%d_b%dc%d_mv%d.txt", TUNER_VERSION, gpuNameForFile.c_str(), nnXLen, nnYLen, model->trunk.numBlocks, model->trunk.trunkNumChannels,model->version);
}
