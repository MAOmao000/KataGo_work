...
//      compiledProgramsByDeviceId[device->info.deviceId] = compiledPrograms;
      compiledProgramsByDeviceId[i] = compiledPrograms;
    }
  }
...
//std::cout << "initialConv->apply() start " << std::this_thread::get_id() << std::endl;
...
//std::cout << "initialMatMul->apply() start " << std::this_thread::get_id() << std::endl;
...
//std::cout << "addChannelBiases() start " << std::this_thread::get_id() << std::endl;
    ...
    for(int i = 0; i<blocks.size(); i++) {
      ...
      if(blocks[i].first == ORDINARY_BLOCK_KIND) {
        ResidualBlock* block = (ResidualBlock*)blocks[i].second;
//std::cout << "block->apply() ORDINARY_BLOCK_KIND start " << std::this_thread::get_id() << std::endl;
        block->apply(
          handle,
          batchSize,
        ...
        GlobalPoolingResidualBlock* block = (GlobalPoolingResidualBlock*)blocks[i].second;
//std::cout << "block->apply() GLOBAL_POOLING_BLOCK_KIND start " << std::this_thread::get_id() << std::endl;
        block->apply(
          handle,
          batchSize,
          trunkScratch, //Flip trunk and trunkScratch so that the result gets accumulated in trunkScratch
...
//std::cout << "trunkTipBN->apply() start " << std::this_thread::get_id() << std::endl;
...
//std::cout << "trunk->apply() start " << std::this_thread::get_id() << std::endl;
    trunk->apply(
      handle,
      batchSize,
      input,
...
//std::cout << "trunk->apply() end   " << std::this_thread::get_id() << std::endl;
    policyHead->apply(
      handle,
      batchSize,
      mask,
...
//std::cout << "gpuHandle->model->apply() start " << std::this_thread::get_id() << std::endl;
...
//std::cout << "gpuHandle->model->apply() end   " << std::this_thread::get_id() << std::endl;
